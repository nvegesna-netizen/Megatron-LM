...
def _make_config(*, q_lora_rank=32, mla_layernorm_epsilon=1.0e-6):
    return MLATransformerConfig(
        ...
        mla_layernorm_epsilon=mla_layernorm_epsilon,
    )
...
@pytest.mark.parametrize("mla_layernorm_epsilon", [None, 1.0e-6])
def test_mla_layernorm_epsilon_default(mla_layernorm_epsilon):
    config = _make_config(mla_layernorm_epsilon=mla_layernorm_epsilon)
    expected = config.layernorm_epsilon if mla_layernorm_epsilon is None else mla_layernorm_epsilon

    assert config.mla_layernorm_epsilon == expected


def test_mla_layernorm_epsilon_argument():
    parser = ArgumentParser()
    _add_mla_args(parser)

    args = parser.parse_args(["--mla-layernorm-epsilon", "1e-6"])

    assert args.mla_layernorm_epsilon == pytest.approx(1.0e-6)


@pytest.mark.parametrize(
    ("q_lora_rank", "q_projection"), [(None, "linear_q_proj"), (32, "linear_q_up_proj")]
)
def test_fused_norm_projections_carry_mla_epsilon(q_lora_rank, q_projection):
    ...
    assert q_spec.params["eps"] == config.mla_layernorm_epsilon

    kv_spec = resolved["linear_kv_up_proj"]
    assert isinstance(kv_spec, ModuleSpec)
    assert kv_spec.module is _FusedNormLinear
    assert kv_spec.params == {"custom": True, "eps": config.mla_layernorm_epsilon}
    assert submodules.linear_kv_up_proj.params == {"custom": True}


def test_dsa_mla_layernorm_epsilon_matches_native():
    """Exercise distinct global, MLA latent, and indexer norm epsilons in DSA."""
    run_absorbed_mla_dsa_parity(
        kernel_backend="none",
        seqlen=64,
