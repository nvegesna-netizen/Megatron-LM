...
class QKNormConfigResolver:
    ...
    def _mla_fused_linear_or_default(self, module_spec, module_name):
        ...
        return self._with_mla_norm_epsilon(fused_linear)

    def _with_mla_norm_epsilon(self, module_spec):
        """Attach MLA's latent-norm epsilon to a fused norm+linear projection."""
        assert self.config.mla_layernorm_epsilon is not None
        params = {
            **(module_spec.params if isinstance(module_spec, ModuleSpec) else {}),
            "eps": self.config.mla_layernorm_epsilon,
        }
